        ...
        fetch(`/api/followsTo/${userId}`)
            ...
            .then(data => {
                ...
                for (let i = 0; i < data.length; i++) {
                    // console.log(data[i]);
                    fetch(`/api/followsTo/findUser/${data[i].FollowId}`).then(res => res.json()).then(res2 => {
                        console.log(`I am a following !! i : ${i}`);
                        let following_1 = res2.username;
                        console.log(following_1);
                        const listItem = document.createElement('li');
                        listItem.textContent = following_1;
